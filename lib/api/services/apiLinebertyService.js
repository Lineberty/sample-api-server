...
const {validateQuery} = require('../middleware/validatorMiddleware')
...
router.get('/apiKey', async (req, res) => {
    try {
        ...
        log('Get : Error while getting apiKey : %o', {errName: err.name, error: err})
...
router.get('/login', validateQuery(schema.logUser), async (req, res) => {
...
router.get('/refreshToken', validateQuery(schema.refreshToken), async (req, res) => {
    ...
    try {
        ...
        log('Get : Error while refreshing user token : %o', err)
        return res.status(500).json({})
    }
})
